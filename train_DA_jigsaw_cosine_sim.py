...
class Trainer:
    ...
    def _do_epoch(self, logger):
        feat1 = torch.ones([32, 64, 111, 111])
        ...
        for it, (source_batch, target_batch) in enumerate(zip(self.source_loader, itertools.cycle(self.target_jig_loader))):
            ...
            jigsaw_logit, class_logit, x_conv1 = self.model(data)
            
            target_jigsaw_logit, target_class_logit, x_conv1_t = self.model(tdata)
            # breakpoint()
            ...
            if it%10==0:
                print("---------------", cos_sim.sum(), "---------------------", cos_sim_t.sum())
                logger.info(str(cos_sim.sum()) + "<-cos sim stability, cos sim with ir ->" + str(cos_sim_t.sum()) + "Epoch no: " + str(self.current_epoch) + "," +  " jigsaw_loss " + string_jigsaw_loss + " class_loss " + string_class_loss + " jigsaw prediction " + string_jigprediction + " class prediction " + string_cls_prediction)
...
# Create the plot
        ...
        plt.show()
        plt.savefig("./cos_sim_list_bs_32_mscoco_run3"+ str(self.current_epoch) +".jpg")
        plt.close()
        plt.plot(cos_sim_ref_list)
        plt.show()
        plt.savefig("./cos_sim_ref_list_bs_32_mscoco_run3"+ str(self.current_epoch) +".jpg")
        plt.close()
