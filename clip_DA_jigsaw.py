...
from models import model_factory_new as model_factory
...
class Clipmodel(nn.Module):
    def __init__(self, source_cls_model_backbone):
        super().__init__()
        # self.clip_model = clip_model
        # self.image_encoder = clip_model.visual
        # self.classnames_desc = classnames_desc
        # self.source_cls_model = source_cls_model
        # self.source_cls_model_cls = source_cls_model_cls
        self.source_cls_model_backbone= source_cls_model_backbone
        self.classifier = nn.Linear(2048, 6)
        self.jig_classifier = nn.Linear(2048, 31)
        self.avgpool = nn.AvgPool2d(13,stride=1)

    def forward(self, image, label=None, training=True):
        
        image_features = self.source_cls_model_backbone(image)
        ...
        jig_logits = self.jig_classifier(image_features)
        return cls_logits, jig_logits
        # else:
        #     # breakpoint()
        #     logits = self.source_cls_model_cls(image_features)
        #     return logits
...
class Trainer:
    def __init__(self, args, device):
        ...
        self.device = "cuda"
        # breakpoint()
        model = network.modeling.deeplabv3plus_resnet_clip(num_classes=6, BB="RN50",replace_stride_with_dilation=[False,False,True])
        # breakpoint()
    ...
    def _do_epoch(self, logger):
        ...
        for it, (source_batch, target_batch) in enumerate(zip(self.source_loader, itertools.cycle(self.target_jig_loader))):
            ...
            class_logit, jigsaw_logit= self.clip(data)
            
            target_class_logit, target_jigsaw_logit= self.clip(tdata)
            # breakpoint()
            ...
            # x_conv1  = x_conv1/x_conv1.norm(dim=-1, keepdim=True)
            # feat1  = feat1/feat1.norm(dim=-1, keepdim=True)
            # cos_sim = feat1 - x_conv1
            # cos_sim = cos_sim/cos_sim.norm(dim=-1, keepdim=True)
            
            # x_conv1_t = x_conv1_t/x_conv1_t.norm(dim=-1, keepdim=True)
            # cos_sim_t = x_conv1_t - x_conv1
            # print(cos_sim_t.sum())
            # cos_sim_t = cos_sim_t/cos_sim_t.norm(dim=-1, keepdim=True)
            # print("---------------", cos_sim.sum(), "---------------------", cos_sim_t.sum())



            # cos_sim_ref = func.cosine_similarity(x_conv1, feat1, dim=-1)
            # cos_sim = func.cosine_similarity(x_conv1, x_conv1_t, dim=-1)
            # print(cos_sim.sum(), "--------------------", cos_sim_ref.sum())

            
            # cos_sim_list = self.cos_sim_list
            # cos_sim_ref_list = self.cos_sim_ref_list

            # cos_sim_list.append(cos_sim.cpu().detach().sum())
            # cos_sim_ref_list.append(cos_sim_t.cpu().detach().sum())
            # print(cos_sim.sum(), "--------------------", cos_sim_ref.sum())
            ...
            loss = class_loss + jigsaw_loss + target_jigsaw_loss * self.target_weight + target_entropy_loss * self.target_entropy
...
# Create the plot for cosine similarity
        ...
        # plt.show()
        # plt.savefig("./cos_sim_list"+ str(self.current_epoch) +".jpg")
        # plt.close()
        # plt.plot(cos_sim_ref_list)
        # plt.show()
        # plt.savefig("./cos_sim_ref_list"+ str(self.current_epoch) +".jpg")
        # plt.close()
...
# For loading checkpoint and inferencing................
        # model = self.clip
        # checkpoint = torch.load("./outputs/clip_fine_tune_jigsaw_m3fd_run1/epoch14.pth")
